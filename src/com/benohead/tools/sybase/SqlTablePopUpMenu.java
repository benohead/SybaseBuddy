...
class SqlTablePopUpMenu extends JPopupMenu {
	...
	public SqlTablePopUpMenu(final JFrame frame, SqlTable sqlTable, int column, int row,
			Hashtable<String, ArrayList<ContextMenuItem>> contextMenuItems) {
		...
		if (value != null) {
			...
			for (Iterator<ContextMenuItem> iterator = menu.iterator(); iterator.hasNext();) {
				...
				if (contextMenuItem.action.equalsIgnoreCase("execute")) {
					...
					anItem.addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent arg0) {
							try {
								int result = JOptionPane.showConfirmDialog(frame, "Are you sure you want to " + menuItemText + " ?", menuItemText,
